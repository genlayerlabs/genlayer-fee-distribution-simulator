...
def hash_variant(variant: VariantConfig) -> str:
    """Hash a variant config including rotation/idle parameters."""
    parts = ["-".join(variant.path)]
    if variant.rotation_counts:
        rot_str = ",".join(f"{k}:{v}" for k, v in sorted(variant.rotation_counts.items()))
        parts.append(f"rot={rot_str}")
    if variant.idle_config:
        idle_str = ",".join(f"{k}:{v}" for k, v in sorted(variant.idle_config.items()))
...
def process_path(
    path: List[str],
    addresses: List[str],
    rotation_counts: Dict[int, int] = None,
    idle_config: Dict[int, int] = None,
    ...
    # Use variant-aware hash if rotation/idle present
    variant = VariantConfig(path=path, rotation_counts=rotation_counts, idle_config=idle_config)
    result_data = {
        "path": path_indices,
        "labels": label_indices,
        "participants": active_participants,
        "hash": hash_variant(variant),
    }
    # Include variant info if non-default
    if rotation_counts:
        result_data["rotation_counts"] = rotation_counts
    if idle_config:
        result_data["idle_config"] = idle_config
    ...
    return result_data, invariant_data, total_rotations


def generate_filename(variant: VariantConfig) -> str:
    intermediate_nodes = len(variant.path) - 2
    variant_hash = hash_variant(variant)[:8]
    return f"{intermediate_nodes:02d}-{variant_hash}.json"
...
def main():
    ...
    parser.add_argument(
        "--max-rotations", type=int, default=0,
        help="Max leader timeout rotations per normal round (default: 0, no variants)"
    )
    parser.add_argument(
        "--max-idle", type=int, default=0,
        help="Max idle validators per normal round (default: 0, no variants)"
    )
    ...
    has_variants = args.max_rotations > 0 or args.max_idle > 0
    ...
    if has_variants:
        console.print(
            f"[cyan]Variant expansion[/cyan]: max_rotations={args.max_rotations}, max_idle={args.max_idle}"
        )
    ...
    # --- Path Counting ---
    total_paths, total_valid_paths, total_variants = 0, 0, 0
    ...
    if has_variants:
        count_table.add_column("Variants", justify="right")
    ...
    for edge_count in range(2, args.max_length + 1):
        ...
        variant_count = sum(
            count_variants(p, args.max_rotations, args.max_idle) for p in valid_paths
        ) if has_variants else valid_count
        total_variants += variant_count

        rate = f"{(valid_count/path_count*100):.1f}%" if path_count > 0 else "N/A"
        row = [str(intermediate_nodes), f"{valid_count:,}", f"{path_count:,}", rate]
        if has_variants:
            row.append(f"{variant_count:,}")
        count_table.add_row(*row)

    console.print(count_table)
    items_to_process = total_variants if has_variants else (total_valid_paths if not args.no_filter else total_paths)
    label = "variants" if has_variants else "paths"
    console.print(
        f"\n[bold]Total {label} to process: [cyan]{items_to_process:,}[/cyan][/bold]"
    )
    ...
    with Progress(
        ...
        task = progress.add_task("Processing...", total=items_to_process)
        ...
        for edge_count in range(2, args.max_length + 1):
            ...
            for path in paths_to_process:
                # Generate variants for this path (or just the base path if no variant expansion)
                if has_variants:
                    variants = list(generate_variants(path, args.max_rotations, args.max_idle))
                else:
                    variants = [VariantConfig(path=path, rotation_counts={}, idle_config={})]
                ...
                for variant in variants:
                    ...
                    try:
                        result_data, invariant_data, total_rotations = process_path(
                            variant.path, addresses,
                            rotation_counts=variant.rotation_counts,
                            idle_config=variant.idle_config,
                        )
                        filename = generate_filename(variant)
                        filepath = length_dir / filename

                        try:
                            invariant_bitfield = check_invariants(
                                *invariant_data, total_rotations=total_rotations,
                            )
                        ...
                        if invariant_exception:
                            errors += 1
                            error_text = Text(
                                f"\nError processing variant {' -> '.join(variant.path)} "
                                f"rot={variant.rotation_counts} idle={variant.idle_config}: "
                                f"{invariant_exception}"
                            )
                        ...
                        filepath = length_dir / filename
                        console.print(
                            f"\n[bold red]Critical error processing variant "
                            f"{variant.path} rot={variant.rotation_counts} "
                            f"idle={variant.idle_config}: {e}[/bold red]"
                        )
                        console.print(
                            f"  -> [bold]Check generated file for details:[/] {filepath}"
                        )

...
def decode_path(
    compressed_data: Dict, lookup_tables: Dict
) -> Tuple[List[str], List[RoundLabel]]:
...
def decode_roles(
    role_indices: List[List[int]], lookup_tables: Dict
) -> List[Tuple[int, str]]:
...
def reconstruct_transaction(
    compressed_data: Dict, lookup_tables: Dict, addresses: List[str]
...
def display_compressed_data(compressed_data: Dict, lookup_tables: Dict):
    ...
    print("\nParticipant Summary:")
    print(
        f"{'Addr':>6} {'Rounds':>10} {'Cost':>10} {'Earned':>10} {'Slashed':>10} {'Burned':>10}"
    )
    print("-" * 66)

    for addr_idx, data in sorted(
        compressed_data["participants"].items(), key=lambda x: int(x[0])
    ):
        rounds = len(data["r"])
        print(
            f"{addr_idx:>6} {rounds:>10} {data['c']:>10} {data['e']:>10} {data['s']:>10} {data['b']:>10}"
        )
    ...
    print("-" * 66)
    print(
        f"{'TOTAL':>6} {'':>10} {total_cost:>10} {total_earned:>10} {total_slashed:>10} {total_burned:>10}"
    )
...
def main():
    ...
    parser.add_argument("json_file", type=str, help="Path to the JSON file to decode")
    parser.add_argument(
        "--json-dir",
        type=str,
        default="path_jsons",
    ...
    parser.add_argument(
        "-a", "--show-all", action="store_true", help="Show all visualizations"
    )
    ...
    if not any(
        [
            args.show_compressed,
            args.show_transaction,
            args.show_fees,
            args.show_summary,
            args.show_all,
        ]
    ...
    if args.show_transaction or args.show_fees or args.show_summary or args.show_all:
        ...
        transaction_results, transaction_budget, full_addresses = (
            reconstruct_transaction(compressed_data, lookup_tables, addresses)
        )

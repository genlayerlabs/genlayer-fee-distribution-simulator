...
from tests.round_combinations.path_types import Path, NodeName
...
def get_appeal_round_size(state: AddressAllocationState, prev_was_unsuccessful: bool) -> int:
...
def simulate_normal_round(state: AddressAllocationState) -> Optional[int]:
    ...
    if state.normal_count == 0:
        ...
        available = state.cumulative_active - set(state.previous_leaders) - state.removed_addresses
...
def simulate_appeal_round(state: AddressAllocationState, prev_was_unsuccessful: bool) -> Optional[int]:
    """
...
def apply_round_to_state(state: AddressAllocationState, node: NodeName, prev_node: Optional[NodeName]) -> bool:
    """
    ...
    if is_normal_round(node):
        ...
        if state.normal_count == 0:
            ...
            new_indices = set(range(state.next_unused_idx, state.next_unused_idx + round_size))
            state.next_unused_idx += round_size
        else:
            # Reuse cumulative minus leaders
            available = list(state.cumulative_active - set(state.previous_leaders) - state.removed_addresses)
            available.sort()
...
def get_path_statistics(path: Path, max_addresses: int = 1000) -> Optional[Dict[str, any]]:
    """
    ...
    for i, node in enumerate(path):
        ...
        if is_normal_round(node):
            ...
            temp_state.normal_count = sum(1 for detail in round_details if detail["type"] == "normal")
            ...
            temp_state = AddressAllocationState(max_addresses)
            temp_state.appeal_count = sum(1 for detail in round_details if detail["type"] == "appeal")
        ...
        round_details.append({
            ...
            "is_unsuccessful": is_unsuccessful_appeal(node) if round_type == "appeal" else None
        })
...
def analyze_path_distribution(paths: List[Path], max_addresses: int = 1000) -> Dict[str, any]:
    """
...
if __name__ == "__main__":
    ...
    test_paths = [
        ...
        # Path with appeal
        ["START", "LEADER_RECEIPT_MAJORITY_AGREE", "VALIDATOR_APPEAL_SUCCESSFUL", "END"],
